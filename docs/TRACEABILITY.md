...
| ban-enums | Ban enums; use as const objects with type unions | M | both | type-declarations.md | typescript-narrows/ban-enums | covered |
...
| consistent-type-imports | Use import type for type-only imports | S | both | type-declarations.md | @typescript-eslint/consistent-type-imports | covered |
| constrain-generics | Always constrain generic type parameters | M | skill-only | generics.md | -- | skill-only |
| error-discrimination | Use discriminated union errors, not string messages | M | skill-only | error-handling.md | -- | skill-only |
...
| exhaustive-switch | Handle all cases in discriminated union switches | B | both | null-handling.md | @typescript-eslint/switch-exhaustiveness-check | covered |
...
| no-non-null-assertion | Do not use the ! postfix operator | B | both | type-safety.md | @typescript-eslint/no-non-null-assertion | covered |
...
| prefer-interface | Use interfaces for object shapes, types for unions/intersections/utilities | M | both | type-declarations.md | @typescript-eslint/consistent-type-definitions | covered |
